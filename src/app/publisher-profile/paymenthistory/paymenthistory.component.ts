...
export class PaymenthistoryComponent implements OnInit{
  // displayedColumns: any[] = ['Book','userName','userEmail','page_no','amount','date'];
  displayedColumns: any[] = ['Book','page_no','amount','date'];
// @ViewChild('logform') logForm!:NgForm
  ...
  // ngOnInit(): void {
  // }
  constructor(private payment:PaymentServiceService,private toastr:ToastrManager) { }
  ...
  fetchdata(){
   this.payment.getPublisherpaymenthistory(localStorage.getItem('pub-id'),'','').subscribe(data=>{
     console.log(data); 
        this.Payment=data;
        if(this.Payment.message.length > 0){
         for(let i=0;i<this.Payment.message.length;i++){
          this.base_dt.push({book_name:this.Payment.message[i].book_details.book_name,book_page_no:this.Payment.message[i].book_page_no
            ,total_price:this.Payment.message[i].total_price,date:this.Payment.message[i].date});
         }
        }
  ...
  submit(v:NgForm){
    ...
    this.loading=false;this.toastr.errorToastr('Start date must not exceed end date','');
      ...
      this.payment.getPublisherpaymenthistory(localStorage.getItem('pub-id'),v.form.value.startDate,v.form.value.enddate).subscribe(data=>{
        //  this.loading=false;
        // console.log(data);
